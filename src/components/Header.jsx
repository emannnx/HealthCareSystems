...
const Header = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setIsScrolled(currentScrollY > 60 && currentScrollY > lastScrollY);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const fetchSuggestions = async () => {
        setLoading(true);
        try {
          const res = await fetch(`https://searchcondition.onrender.com/searches/get/${query.trim()}`);
          if (!res.ok) throw new Error('No results');
          const data = await res.json();
          const names = Array.isArray(data) ? data.map(i => i.name) : [data.name];
          setFilteredSuggestions(names);
          setError('');
        } catch (err) {
          setFilteredSuggestions([]);
  ...
  useEffect(() => {
    const fetchAllConditions = async () => {
      ...
      setLoading(true);
      try {
        const res = await fetch('https://searchcondition.onrender.com/searches/getAll');
  ...
  return (
    <header className={`header ${darkMode ? 'dark-mode' : ''} ${isScrolled ? 'scrolled-header' : ''}`}>
      <nav className="header-nav">
        <div className="header-container">
          <div className="header-inner">
            <div className="header-left">
              <Link to="/" className="logo-container">
                <div className="logo-link">
                  <img src={img} className='iconss' alt="logo" />
                  <span className="logo-primary">Health</span>
                  <span className="logo-secondary">Hub</span>
                </div>
              </Link>
            ...
            <div className="header-right">
              <div className="search-container" onClick={() => setDialogOpen(true)}>
                <svg xmlns="http://www.w3.org/2000/svg" className="search-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <circle cx="11" cy="11" r="8" />
                  <path d="m21 21-4.3-4.3" />
                </svg>
              ...
              {isAuthenticated && (
                <Link to="/profile" className="profile-link">
                  <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="grey" strokeWidth="2">
                    <path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2" />
                    <circle cx="12" cy="7" r="4" />
                  </svg>
                </Link>
              )}
              <button className="signin-button" onClick={isAuthenticated ? handleSignOut : () => setSigninOpen(true)}>
                {isAuthenticated ? 'Sign Out' : 'Sign In'}
              </button>
              <button className="menu-toggle" onClick={toggleMenu} aria-label="Toggle menu">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <div className="search-popup-overlay">
          <div className="search-popup">
            <div className="popup-header">
              <div className="search-input-container">
                <div className="input-wrapper">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="searches-icons">
                    <circle cx="11" cy="11" r="8" />
                    <path d="m21 21-4.3-4.3" />
                  </svg>
                  <input type="text" id="health-topic-search" className="search-input" placeholder="Search health topics..." value={query} onChange={(e) => setQuery(e.target.value)} />
                </div>
                <button onClick={() => setDialogOpen(false)} className="close-popup">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M18 6 6 18" />
                    <path d="m6 6 12 12" />
                  </svg>
              ...
              {loading ? (
                ...
                <ul className="suggestion-list">
                  {filteredSuggestions.map((s, i) => (
                    <li key={i} onClick={() => { navigate(`/health-search?query=${encodeURIComponent(s)}`); setDialogOpen(false); }} className="search-result-item">{s}</li>
                  ))}
                </ul>
              ) : (
                error && <div className="error-message">{error}</div>
